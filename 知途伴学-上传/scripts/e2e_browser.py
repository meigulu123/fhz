...
class Driver:
    ...
    def goto_page(self, label, wait=1.4):
        self.page.locator('[data-testid="stSidebarNav"]').locator(
            f'a:has-text("{label}")').first.click()
...
def main():
    ...
    with sync_playwright() as p:
        ...
        page.get_by_text("核心能力").last.wait_for(timeout=30000)
        ...
        if node_name:
            # 路径表格是 canvas 渲染，节点名只在「为什么这样排」展开栏里以文本出现
            page.get_by_text("为什么这样排").click()
            page.wait_for_timeout(400)
            ok_path = ok_path and node_name in page.inner_text(MAIN)
        check("学习路径页", ok_path, f"当前节点「{node_name}」在路径中" if node_name else "")
        d.goto_page("知识图谱")
